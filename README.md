...
> 為 **zenbu org** 量身打造的 Claude Code Plugin。內建 Orchestrator 心法注入、22 個專業 Agent、145+ 個 Skill，覆蓋 WordPress 全棧、React / NestJS / Node.js 現代應用、AIBDD 行為驅動開發三大領域，讓 AI 工程師團隊從第一個 session 就知道如何分工。

**v3.3.1** — 完整支援 AIBDD TypeScript / C# / PHP 三語言整合測試套件；架構升級為模組化 Orchestrator-Agent-Skill 三層委派體系。
...
4. 重新啟動 Claude Code session
5. 若仍無效，可手動執行 `/using-zenbu-powers` 載入 Orchestrator 心法
