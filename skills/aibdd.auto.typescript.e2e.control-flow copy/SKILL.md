---
name: aibdd.auto.typescript.e2e.control-flow
description: TypeScript E2E 全自動批次迴圈。掃描 features 目錄，為每個 .feature 展開完整的 5 phase TODO 清單（msw-api-layer → step-template → red → green → refactor），然後逐一執行直到全數完成。
user-invocable: true
...
```
content: "01-增加影片進度.feature — MSW API Layer"
activeForm: "建立 MSW API Layer：01-增加影片進度.feature"
status: pending
...
| 任務 phase | 呼叫的 Skill |
|-----------|-------------|
| MSW API Layer | `/aibdd.auto.frontend.msw-api-layer` |
| Step Template | `/aibdd.auto.typescript.e2e.step-template` |
| Red | `/aibdd.auto.typescript.e2e.red` |
| Green | `/aibdd.auto.typescript.e2e.green` |
| Refactor | `/aibdd.auto.typescript.e2e.refactor` |
