...
當 `@wp-workflows:doc-manager` 判定為 greenfield（`.claude/CLAUDE.md` 存在但 `specs/` 不存在；或透過其他啟發式判斷）時，載入本 skill 並依 references/ 順序執行。
...
- **Library SKILL**：scaffolding 過程中若發現複雜依賴，委派 `@wp-workflows:lib-skill-creator`。
- **Specs 完善**：specs 草稿完成後，委派 `@wp-workflows:clarifier` 進行 discovery / clarify-loop。
- **合規審查**：所有文件就緒後，由 doc-manager 呼叫 `@wp-workflows:claude-manager` 做合規審查。
- **Incremental 場景**：本 skill 不負責增量更新；該場景由 doc-manager 直接委派給 `@wp-workflows:doc-updater` agent。
