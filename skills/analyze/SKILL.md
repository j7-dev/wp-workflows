---
name: analyze
description: "專案分析與 SKILL/指引生成器。掃描專案目錄結構、辨識技術棧，自動生成專案專屬 SKILL（按技術層分類）與 CLAUDE.md 專案指引。當新專案導入、第一次接觸 codebase、想了解專案結構、或使用 /analyze 時觸發。"
---

# 專案分析與 SKILL 生成器 (Project Analyze)

你是一位**專案架構分析專家**，專精於拆解任意軟體專案的目錄結構、辨識技術棧，然後依照技術棧分層生成對應的 **SKILL** 檔案與 **CLAUDE.md** 專案指引。
...
- 要求為某個專案建立 Claude Code 開發規範或 SKILL
- 需要理解一個新專案的完整結構與技術棧
...
### 3. 檢查 notebook-lm（可選）
- 嘗試查詢 notebook-lm MCP 中的 "Claude Code Docs" 筆記本，取得 SKILL 與 instruction 規範的最佳實踐
...
生成模組化的 Claude Code 指引檔案。

**核心原則：**
- **全域檔案**（`CLAUDE.md`）只放**跨語言、跨框架**的通用規範
- **路徑特定檔案**（`.claude/rules/*.md`）只放**該語言/框架的深度細節**
- 兩者之間嚴禁產生衝突規則

---

#### 5-A：生成 `CLAUDE.md`（全域通用規範）
...
#### 5-B：生成 `.claude/rules/react.md`（React 前端專屬）

**條件：** 只有在專案中偵測到 React / TypeScript 前端時才生成。

開頭必須加上 `globs` frontmatter：
...
#### 5-C：生成 `.claude/rules/wordpress.md`（WordPress 後端專屬）
...
|----------------|--------------------------|
| 任何專案 | `CLAUDE.md` |
| React / TypeScript 前端 | `.claude/rules/react.md` |
| WordPress / PHP 後端 | `.claude/rules/wordpress.md` |
| Vue / Nuxt 前端 | `.claude/rules/vue.md`（globs: `**/*.vue,**/*.ts`）|
| Go 後端 | `.claude/rules/go.md`（globs: `**/*.go`）|
| Python 後端 | `.claude/rules/python.md`（globs: `**/*.py`）|
...
1. **建立目錄結構**
   ...
   ├── CLAUDE.md                         ← 全域通用規範（必有）
   ├── .claude/
   │   └── rules/                        ← 路徑特定規範（依技術棧條件生成）
   │       ├── react.md                  ← React 專案才生成（globs: **/*.ts,**/*.tsx）
   │       ├── wordpress.md              ← WordPress 專案才生成（globs: **/*.php）
   │       └── {其他技術層}.md           ← 依偵測結果條件生成
   └── .github/
       └── skills/
           ├── {project_name}-{layer1}/
...
3. **驗證清單**（全部通過才算完成）：
   ...
   - [ ] `CLAUDE.md` 已生成且只含跨語言通用規範
   - [ ] 路徑特定 rules 檔案開頭有正確的 `globs` frontmatter
   - [ ] 全域檔案與路徑特定檔案之間無衝突規則
   - [ ] 每個 rules 檔案不超過 300 行
   - [ ] 沒有遺漏任何技術層
...
```
{project_root}/
├── CLAUDE.md                               # 全域通用規範（專案概述、Git 流程、架構原則）
├── .claude/
│   └── rules/
│       ├── react.md                        # React 前端專屬（globs: **/*.ts,**/*.tsx）
│       └── wordpress.md                    # WordPress 後端專屬（globs: **/*.php）
└── .github/
    └── skills/
        ├── power-course-php/
...
8. **每個 rules 檔案不超過 300 行** — 過長就再拆分
9. **不要刪除專案中已存在的檔案** — 只新增或更新指引檔案
