---
name: wp-e2e-creator
description: 根據 `./specs` 規格文件，識別邊緣案例並生成 Playwright E2E 測試。涵蓋邊緣案例識別框架、測試生成模式，以及 wp-env + Playwright 技術實作。當使用者需要為 WordPress Plugin 建立 E2E 測試、識別測試邊緣案例、或設定 wp-env + Playwright 環境時，請啟用此技能。
---

# WordPress Plugin E2E 測試生成器

從 `./specs` 規格到測試生成的完整流程：**讀取 `./specs` 規格 → 識別邊緣案例 → 生成 Playwright E2E 測試**。
...
> 提示使用者先執行需求訪談，產生 `./specs/` 規格文件後再繼續。
...
```
Step 1: 讀取 `./specs/`，建立情境清單
  └── 從 `./specs/` 提取使用者角色、功能情境、業務規則
...
1. **情境先行** — 先從 `./specs/` 分析找出所有情境，再決定要測試哪些
