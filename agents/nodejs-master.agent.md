...
```
@agents/nodejs-reviewer.agent.md
```
...
當 `@agents/nodejs-reviewer.agent.md` 審查不通過並將意見退回時，你必須：
...
5. **再次提交審查**：測試通過後，再次呼叫 `@agents/nodejs-reviewer.agent.md` 進行審查

```
修改完成 → 跑測試 → 全部通過 → @agents/nodejs-reviewer.agent.md
```
