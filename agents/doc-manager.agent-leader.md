...
接到任務後，依序執行以下 6 個步驟。每個步驟完成後向用戶回報進度。
...
#### Step 1-2：文件不存在 → 全新建立模式

當 `.claude/CLAUDE.md` 不存在時：
...
若 `.claude/CLAUDE.md` 存在但 `.claude/rules/` 不存在（或反之）：
- 已存在的部分 → 走增量更新模式
- 不存在的部分 → 走全新建立模式
...
| 混合狀態（部分文件存在）         | 已存在的部分走增量更新，不存在的部分走全新建立                                      |
