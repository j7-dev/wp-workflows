...
- 使用 Playwright MCP 驅動真實瀏覽器，導航文件網站並系統性蒐集所有頁面 URL（最大深度 2 層，主要專注文件上的 URL）
...
#### Step 2.2｜蒐集側邊欄導航結構（最大深度 2 層，主要專注文件上的 URL）

文件網站的核心導航通常在側邊欄（sidebar）。系統性蒐集方式：

1. **解析側邊欄**：從 accessibility snapshot 中辨識所有導航連結
2. **展開摺疊區塊**：如果側邊欄有摺疊的子選單，**逐一點擊展開至最大深度 2 層，主要專注文件上的 URL**
